...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<RestErrorMessage> productNotFoundException(ProductNotFoundException exception){
        RestErrorMessage response = new RestErrorMessage(HttpStatus.NOT_FOUND, Collections.singletonList(exception.getMessage()));
    ...
    private ResponseEntity<RestErrorMessage> productAlreadyExistsException(ProductAlreadyExistsException exception){
        RestErrorMessage response = new RestErrorMessage(HttpStatus.CONFLICT, Collections.singletonList(exception.getMessage()));
        return ResponseEntity.status(HttpStatus.CONFLICT).body(response);
    }
    @ExceptionHandler(ValidationException.class)
    private ResponseEntity<RestErrorMessage> validationException(ValidationException exception){
        RestErrorMessage response = new RestErrorMessage(HttpStatus.BAD_REQUEST, Collections.singletonList(exception.getMessage()));
        return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(response);
    }
    @ExceptionHandler(DatabaseAccessException.class)
    private ResponseEntity<RestErrorMessage> databaseAccessException(DatabaseAccessException exception){
        RestErrorMessage response = new RestErrorMessage(HttpStatus.INTERNAL_SERVER_ERROR, Collections.singletonList(exception.getMessage()));
